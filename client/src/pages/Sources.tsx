...
export default function Sources() {
  ...
  return (
    <>
      <Seo title="Source library" description="Browse CasinooVerse-owned source records with publisher, publication, source type, retrieval date, and stored provenance address." path="/sources" />
      <header className="source-library-hero border-b border-gold/15">
        <div className="container grid gap-10 py-16 md:py-24 lg:grid-cols-[1fr_340px] lg:items-end">
          <div><p className="eyebrow text-gold">CasinooVerse internal source library</p><h1 className="mt-4 max-w-5xl font-display text-[clamp(4rem,9vw,8rem)] leading-[.84] tracking-[-.045em] text-ivory">The evidence stays in the house.</h1><p className="mt-7 max-w-3xl text-xl leading-8 text-ivory/62">Every public reference opens a CasinooVerse record. Publisher identity, publication context, source type, retrieval date, and the original address are preserved without redirecting readers to another website.</p></div>
          <aside className="research-method-card"><Database className="h-6 w-6 text-gold" /><h2 className="mt-4 font-display text-3xl text-ivory">Project-owned database</h2><p className="mt-4 text-sm leading-7 text-ivory/58">The source catalogue is stored in CasinooVerse’s own application database. Original addresses are provenance text, not clickable outbound links.</p></aside>
        </div>
      </header>
