...
const standards = [
  ["Trace the claim", "Research stories retain original publisher names, headlines, URLs, publication dates when visible, and the date CasinooVerse accessed the source."],
  ...
  ["Separate information from promotion", "CasinooVerse does not publish bonus rankings, winning systems, betting picks, deposit links, or encouragement to chase profit."],
];
...
export default function About() {
  ...
  return (
    <>
      <Seo title="About CasinooVerse" description="Learn how CasinooVerse researches casino-industry news from Sikkim, India, attributes sources, handles developing stories, and maintains an informational-only editorial standard." path="/about" image={ABOUT_HERO_IMAGE} />
      <header className="about-hero relative overflow-hidden border-b border-gold/15">
        ...
        <div className="container relative flex min-h-[620px] items-end py-16 md:items-center md:py-24">
          <div className="max-w-4xl"><p className="eyebrow text-gold">About the publication</p><h1 className="mt-5 font-display text-[clamp(4rem,9vw,8rem)] leading-[.84] tracking-[-.045em] text-ivory">Context before<br /><em className="font-normal text-gold-light">conclusion.</em></h1><p className="mt-7 max-w-2xl text-xl leading-8 text-ivory/62">CasinooVerse is an independent informational publication based in Sikkim, India, covering casino business, design, regulation, games, destinations, and responsible entertainment.</p></div>
        </div>
      </header>
      ...
      <section className="section-space">
        <div className="container grid gap-12 lg:grid-cols-[.6fr_1.4fr]">
          ...
          <div><h2 className="font-display text-5xl leading-none text-ivory md:text-6xl">Make a complex global industry easier to understand.</h2><p className="mt-7 max-w-3xl text-xl leading-9 text-ivory/58">Casino coverage often sits between finance, law, tourism, technology, entertainment, and public health. CasinooVerse brings those threads together, keeps original sources visible, and writes for readers who want explanation rather than promotion.</p><p className="mt-5 text-xs uppercase tracking-[.14em] text-ivory/35">Standards last reviewed 3 September 2026 · Research desk: CasinooVerse Editorial · Sikkim, India</p></div>
        </div>
      </section>

      <section id="standards" className="scroll-mt-28 border-y border-gold/15 bg-[#11100f] py-20">
        <div className="container">
          <div className="max-w-3xl"><p className="eyebrow text-gold">Editorial standards</p><h2 className="mt-3 font-display text-5xl leading-none text-ivory">How a CasinooVerse story is built.</h2></div>
      ...
      <section id="contact" className="scroll-mt-28 border-y border-white/8 bg-white/[.02] py-20">
        <div className="container grid gap-12 lg:grid-cols-[.8fr_1.2fr]">
          <div><p className="eyebrow text-gold">Contact the desk · Sikkim, India</p><h2 className="mt-4 font-display text-5xl leading-[.95] text-ivory">Corrections, privacy, newsletter, and general questions.</h2><p className="mt-6 max-w-xl leading-8 text-ivory/58">The CasinooVerse editorial desk is based in Sikkim, India. Use this form instead of sending sensitive personal records. The message, contact address, topic, and consent time are stored for editorial follow-up. CasinooVerse does not provide emergency, legal, financial, or treatment advice.</p></div>
          <div className="rounded-[26px] border border-white/10 bg-black/20 p-7 md:p-9">
            {contact.isSuccess ? <div className="flex min-h-[360px] flex-col justify-center" role="status"><CheckCircle2 className="h-8 w-8 text-gold" /><h3 className="mt-5 font-display text-4xl text-ivory">Message recorded.</h3><p className="mt-4 max-w-lg leading-8 text-ivory/60">{contact.data.alreadySubmitted ? "An identical message was already received, so no duplicate was created." : "The editorial desk has received your inquiry."}</p></div> : <form className="grid gap-5 sm:grid-cols-2" onSubmit={event => { event.preventDefault(); contact.mutate({ ...form, consent: true }); }}>
              ...
              <label className="flex cursor-pointer items-start gap-3 text-sm leading-6 text-ivory/58 sm:col-span-2"><input type="checkbox" required checked={form.consent} onChange={event => setForm(current => ({ ...current, consent: event.target.checked }))} className="mt-1 h-4 w-4 accent-[#c9a45c]" /><span>I consent to CasinooVerse storing this inquiry and contact address for editorial follow-up. See the <Link href="/privacy" className="text-gold-light underline underline-offset-4">Privacy Policy</Link>.</span></label>
      ...
      <div className="container pb-24">
        <ResearchReferences
          ...
          intro="CasinooVerse’s internal standards reflect established principles of verification, source transparency, labeling, independence, correction, and accountable feedback. These references guide our method; they do not certify or endorse the publication."
          sources={[
