...
export default function SourceDetail() {
  ...
  return (
    <article>
      ...
      <section className="section-space"><div className="container grid gap-10 lg:grid-cols-[1fr_360px]"><div className="source-record-sheet"><p className="eyebrow text-gold">Stored provenance address</p><h2 className="mt-3 font-display text-4xl text-ivory">Original location retained as data</h2><p className="mt-5 leading-8 text-ivory/58">The address below is shown as non-clickable text so the record remains auditable without redirecting you away from CasinoVerse.</p><code className="source-address mt-7 block">{data.originalUrl}</code></div><aside className="space-y-4"><div className="research-method-card"><CalendarDays className="h-5 w-5 text-gold" /><h2 className="mt-4 font-display text-2xl text-ivory">Record dates</h2><dl className="mt-5 space-y-4 text-sm"><div><dt className="eyebrow text-ivory/35">Accessed</dt><dd className="mt-1 text-ivory/64">{formatDate(data.accessedAt)}</dd></div><div><dt className="eyebrow text-ivory/35">Updated</dt><dd className="mt-1 text-ivory/64">{formatDate(data.updatedAt)}</dd></div></dl></div><div className="research-method-card"><FileText className="h-5 w-5 text-gold" /><h2 className="mt-4 font-display text-2xl text-ivory">Use in reporting</h2><p className="mt-4 text-sm leading-7 text-ivory/55">CasinoVerse uses this record for attribution, fact checking, context, or methodology. The original publisher remains responsible for its own material.</p></div></aside></div></section>
    </article>
  );
}
