    ...
    <section className="research-references" aria-labelledby={`references-${slugify(title)}`}>
      ...
      <ol className="mt-9 grid gap-4 md:grid-cols-2">
        {sources.map((source, index) => (
          <li key={source.href}>
            <a href={source.href} target="_blank" rel="noreferrer noopener" className="research-reference group">
              ...
              <ExternalLink className="h-4 w-4 text-gold/60 transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5" aria-hidden="true" />
            </a>
          </li>
        ))}
      </ol>
